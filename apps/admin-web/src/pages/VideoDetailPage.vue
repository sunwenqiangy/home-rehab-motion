...
async function loadAnalysisDetail() { const id = Number(route.params.videoId); if (!id) return; analysisDetailLoading.value = true; try { analysisDetail.value = await getAdminVideoAnalysisDetail(id); } catch (error: unknown) { analysisDetail.value = null; ElMessage.warning(`评分明细加载失败：${error instanceof Error ? error.message : '未知错误'}`); } finally { analysisDetailLoading.value = false; } }
