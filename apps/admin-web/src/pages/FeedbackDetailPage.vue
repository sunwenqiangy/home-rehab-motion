<template>
  <div class="admin-page feedback-detail-page" v-loading="loading">
        ...
        <el-card class="surface-card" shadow="never">
          ...
          <section class="ticket-timeline">
            <article v-for="message in detail.messages || []" :key="message.messageId" class="ticket-message" :class="`ticket-message--${message.senderRole}`">
              ...
              <p>{{ message.content }}</p>
              <div v-if="message.imageUrls?.length" class="ticket-images"><el-image v-for="(url, index) in message.imageUrls" :key="url" :src="url" fit="cover" @click="openImagePreview(message.imageUrls, index)"><template #error><div class="ticket-images__error">图片加载失败</div></template></el-image></div>
            </article>
    ...
    <Teleport to="body"><div v-if="imagePreviewVisible" class="ticket-image-preview-mask" @click.self="closeImagePreview"><section class="ticket-image-preview-panel" role="dialog" aria-modal="true" aria-label="图片预览"><header><strong>图片预览</strong><small v-if="previewImageUrls.length > 1">{{ previewImageIndex + 1 }} / {{ previewImageUrls.length }}</small><button type="button" aria-label="关闭图片预览" @click="closeImagePreview">×</button></header><div class="ticket-image-preview-panel__body"><button v-if="previewImageUrls.length > 1" class="ticket-image-preview-panel__nav ticket-image-preview-panel__nav--prev" type="button" aria-label="上一张图片" @click="showPreviousImage">‹</button><img v-if="previewImageUrl" :src="previewImageUrl" alt="工单附件图片" @error="previewImageFailed = true"/><div v-if="previewImageFailed" class="ticket-image-preview-panel__error">图片加载失败</div><button v-if="previewImageUrls.length > 1" class="ticket-image-preview-panel__nav ticket-image-preview-panel__nav--next" type="button" aria-label="下一张图片" @click="showNextImage">›</button></div></section></div></Teleport>
  </div>
</template>
...
const analysisStatusLabel = (value?: AnalysisStatus) => value ? ANALYSIS_STATUS_LABELS[value as keyof typeof ANALYSIS_STATUS_LABELS] || value : '未记录';
...
function openImagePreview(urls: string[], index: number) { previewImageUrls.value = urls; previewImageIndex.value = index; previewImageFailed.value = false; imagePreviewVisible.value = true; }
function closeImagePreview() { imagePreviewVisible.value = false; previewImageUrls.value = []; previewImageIndex.value = 0; previewImageFailed.value = false; }
function showPreviousImage() { previewImageIndex.value = (previewImageIndex.value - 1 + previewImageUrls.value.length) % previewImageUrls.value.length; previewImageFailed.value = false; }
function showNextImage() { previewImageIndex.value = (previewImageIndex.value + 1) % previewImageUrls.value.length; previewImageFailed.value = false; }
