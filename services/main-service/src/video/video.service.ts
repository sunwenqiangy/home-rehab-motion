...
callback_last_error: failReason.slice(0, 255),
  ...
  async getAdminAnalysisTasks(options: { page?: number; limit?: number; status?: string; keyword?: string } = {}) {
    ...
    return {
      items: videos.map((video) => ({
        ...
        // retryCount 仅表示自动补偿的入队失败次数，避免混同管理员点击重试。
        retryCount: video.analysis_task?.retry_count || 0,
        manualRetryCount: video.analysis_task?.manual_retry_count || 0,
  ...
  async getAdminVideoAnalysisDetail(videoId: number) {
    ...
    return {
      ...
      segmentation: videoEval
        ? {
            version: videoEval.segmentation_version ?? null,
            snapshot: videoEval.segmentation_snapshot ?? null,
          }
        : null,
      summary: videoEval
        ? {
